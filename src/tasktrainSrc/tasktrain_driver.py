...
from iterators.samp_iter_tasktrain import build_train_loader
from tasktrainer import TaskTrainer
...
def train(hps, trainer, train_loader, rank, run_config, run_name, world_size=1):
    ...
    with CSVLogger(hps["log_dir"] + "metrics.csv") as _csv_logger:
        ...
        for i, (gfn_batch, mols) in enumerate(train_loader):
            try:
                it = i #trainer.start_step+i
                loss, info = tb.compute_batch_losses(trainer.algo, trainer.gfn_trainer,
                                                gfn_batch.to(trainer.device), trainer.device, hps, i)
                # loss, info = algo.compute_batch_losses(gfn_trainer.model, gfn_batch.to(device)) 
                ...
                info_vals = {"Total loss ":loss_val,
                        ...
                        # "Average logP": gfn_batch.avg_logP,
                        ...
                        # "Avg true Mol wt online": gfn_batch.avg_mol_wt_task,
                        # f"Average {hps['task']} online": gfn_batch.avg_task_reward,
                        # "offline_loss": info['offline_loss'].item(),
                ...
                if (hps.objective=='property_targeting'): 
...
def main(rank, world_size):
    ...
    hps = config.tasktraining

    default_hps= {"Z_learning_rate": 1e-3,}
    hps.update(default_hps)

    # Print experiment configuration
    print(f"Running configuration for seed_idx {args.seed_idx}:")
    ...
    if hps.objective=='property_optimization':
        ...
        if hps.subtype=='value_mod':
            conditional_range_dict.update(hps.value_mod.updated_prop)
            hps.task_conditionals = False
            task_conditional_range_dict = None
        elif hps.subtype=='new_props':
            hps.task_conditionals=True
            task_conditional_range_dict =hps.new_props.added_prop
    ...
    hps.update(task_conditional_range_dict)
