...
class GraphBuildingEnv:
    ...
    def __init__(self, allow_add_edge=True, allow_node_attr=True, allow_edge_attr=True, scaffold_masking = False):
        ...
        scaffold_masking: bool
            if True, disallows actions to be taken on masked scaffold (provided by user)
        """
        self.allow_add_edge = allow_add_edge
        self.allow_node_attr = allow_node_attr
        self.allow_edge_attr = allow_edge_attr
        self.graph_cls = None
        self.scaffold_masking = scaffold_masking
    ...
    def step(self, g: Graph, action: GraphAction) -> Graph:
        ...
        # print('graph_build_env.py old graph ',g, 'new graph ',gp)
        return gp
...
class GraphActionCategorical:
    def __init__(
        ...
        """
        # print('gbuildenv.py input graphs ',graphs, ' graphs.num_graphs ',graphs.num_graphs)
        ...
        #     if k is not None:
        #         if k!='x':
        #             attr = getattr(graphs, f"{k}_batch")
        #             self.batch.append(attr)
        #             # print(k, attr)
        #         else:
        #             attr = getattr(graphs, "batch")
        #             self.batch.append(attr)
        #             # print(k, attr)
        #     else:
        #         self.batch.append(torch.arange(graphs.num_graphs, device=dev))
        ...
        if deduplicate_edge_index and "edge_index" in keys:
            ...
            idcs = [i for i,val in enumerate(keys) if val=="edge_index"]
            for idx in idcs:
                self.batch[idx] = self.batch[idx][::2]
                self.slice[idx] = self.slice[idx].div(2, rounding_mode="floor")
            # print('gbuildenv.py after dedup batch ', self.batch)
    ...
    def _compute_batchwise_max(
        ...
        """
        # print('gbuildenv.py _compute_batchwise_max x', x)
        if detach:
            x = [i.detach() for i in x]
        if batch is None:
            batch = self.batch
        # print('gbuildenv.py batch ',batch)
        ...
        # print('gbuildenv.py outs ',outs)
        self.x , self.outs = x,outs
    ...
    def stack(cls, cats):
        cat = cls.__new__(cls)
        cat.num_graphs = sum([i.num_graphs for i in cats])
        cat.logits = [torch.cat([j.logits[i] for j in cats], dim=0) for i in range(len(cats[0].logits))]
        cat.types = cats[0].types
        cat.keys = cats[0].keys
        cat.dev = cats[0].dev
        cat._epsilon = cats[0]._epsilon
        if cats[0].masks is not None:
            cat.masks = [torch.cat([j.masks[i] for j in cats], dim=0) for i in range(len(cats[0].masks))]
        # We want to stack batches of att
        b_offsets = np.cumsum([0] + [i.num_graphs for i in cats])
        cat.batch = [
            torch.cat([c.batch[i] + o for c, o in zip(cats, b_offsets)], dim=0) for i in range(len(cats[0].batch))
        ]
        s_offsets = [
            torch.cumsum(torch.stack([cats[j - 1].slice[i][-1] * (j > 0) for j in range(len(cats))]), 0)
            for i in range(len(cats[0].slice))
        ]
        cat.slice = [
            torch.cat(
                [cats[j].slice[i][0 if j == 0 else 1 :] + s_offsets[i][j] for j in range(len(cats))],
            )
            for i in range(len(cats[0].slice))
        ]
        cat.logprobs = None
